...
class CategoryProductOptionsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["category_option", "value"]

    def create(self, validated_data):
        product = self.context['product_id']
        product_option = CategoryProductOptions.objects.create(product=product, **validated_data)
...
class AddProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ["title", "description", "price", "image", "countInStock","created_by", "category", "category_options"]
